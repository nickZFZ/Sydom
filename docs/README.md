...
| 文档 | 覆盖 | 何时看 |
|---|---|---|
| [企业 SSO（OIDC）配置指南](enterprise-sso.md) | 每租户自带 OIDC IdP：配置（Console/REST）+ 域路由、事前严格映射 vs JIT 自动开通、用户登录体验、运维（轮换/停用/删除/连通性测试/设 operator email）、验签与 fail-close 安全设计、故障排查 | 租户接入企业身份提供方做单点登录，或排查 SSO 登录问题 |
