...
| 决策执行 | `SyncedEnforcer`（RWMutex 线程安全） + `EnforceContext`（多截面） | Sidecar 内核用 `SyncedCachedEnforcer`（内嵌 `SyncedEnforcer`）：`Enforce(ctx, sub, dom, obj, act)` |
| 批量鉴权 | `BatchEnforce` / `BatchEnforceWithMatcher` | 对应 Sidecar `/batch-check` 接口 |
| 决策解释 | `EnforceEx` 返回命中规则列表 | 对应 `/check?explain=true` |
| 决策缓存 | `SyncedCachedEnforcer`（线程安全 + 决策缓存；缓存为裸 `map`，**无 LRU/无容量上界**） | Sidecar 内核（并发鉴权 + 同步协程改策略）；缓存需 `SetCache` 注入有界实现，见 [5 节缓存铁律](#5-数据面-sidecar-架构) |
| 事务批量写 | `TransactionalEnforcer` | 控制面批量策略变更时使用 |
| 策略存储接口 | `persist.Adapter` / `BatchAdapter` / `FilteredAdapter` | **Sydom 实现内存 Adapter**（Sidecar 侧，须同时实现 `FilteredAdapter`），控制面实现 DB Adapter |
| 策略变更通知 | `persist.Watcher` / `WatcherEx` / `UpdatableWatcher` 接口 | **Sydom 实现 gRPC 下发通道**（控制面 → Sidecar 推送）；Sidecar 接收端 enforcer 不挂 watcher，避免回播 |
...
| **内存 Adapter（Sidecar）** | Sidecar 策略从内存缓存读，需实现读内存的 Adapter | `MemoryAdapter`：须同时实现 `persist.BatchAdapter` + `persist.FilteredAdapter`（启动按 domain 分片加载本地快照，`loadFilteredPolicy` 会强制类型断言为 `FilteredAdapter`，否则报错，`enforcer.go:476`）；运行期 `autoSave=false` 不回写 |
...
| 限制 | 影响 | 规避方案 |
|------|------|---------|
| 每请求对每条 policy 规则 `expression.Eval`（O(n) 线性扫描） | 高并发下 CPU 开销大；matcher 编译结果已缓存于 `matcherMap`（`enforcer.go:912`），稳态不重编译，仅策略变更后首请求重编译（`invalidateMatcherMap`） | 决策缓存命中即跳过整轮 Eval；分片把 n 压到单应用量级（见 [3.3 性能注](#33-casbin-的关键限制设计时需规避) 下方与 5 节） |
| 角色图是纯内存结构 | 重启后需从策略重建，大规模时 `BuildRoleLinks()` 耗时 | Sidecar 启动时全量加载；增量更新走高层 `AddPolicies`/`RemovePolicies`（自动 `BuildIncrementalRoleLinks`） |
| Watcher 默认触发全量 `LoadPolicy()` | 高频策略变更时抖动大 | 用 `WatcherEx` 的"广播 delta"语义，接收端自实现增量 apply，只推变更 delta（见 [7 节](#7-策略同步机制) C2） |
...
- **Redis 是 at-most-once，不能作唯一通路**：Redis Pub/Sub 是 fire-and-forget、不持久化——某 Sidecar 正在重连时的 `PUBLISH` 会被静默丢失。因此 **DB 是唯一真相源，Redis 只是快速通知通道**：
  ...
  - **对账**：Sidecar 记录 last-applied 版本号；发现版本不连续（丢包）、收到心跳版本号领先、或 stream 重连时，发起一次**全量快照拉取**（司域自定义 gRPC RPC，只拉本 app 分片，非 casbin `LoadFilteredPolicy`）向控制面对齐——即 [第 7 节](#7-策略同步机制) C2 / R3 已定义的版本号 + 心跳兜底。Redis 丢一条消息，对账兜底会补上，最终一致不破。
...
│        │  (g 段自动 BuildIncrementalRoleLinks) │
│        │  + InvalidateCache (全量清缓存)   │
│  ┌─────▼────────────────────────────┐    │
│  │   casbin SyncedCachedEnforcer     │    │
│  │   (SyncedEnforcer + 决策缓存一体) │    │
│  │   + MemoryAdapter(autoSave=false, │    │
│  │     仅启动加载本应用分片快照)     │    │
│  └─────┬────────────────────────────┘    │
│        │                                 │
│  ┌─────▼────────────────────────────┐    │
...
> 以下指标在**中型基线规模**下成立：单应用 ~10 万条 policy 规则、~1 千角色、~1 万用户；且 Sidecar **按 domain 分片加载**（`LoadFilteredPolicy` 只加载本应用策略）+ 启用 `SyncedCachedEnforcer` 决策缓存。大型规模（~100 万规则）为已知边界，需压测验证。
...
> **决策缓存一致性铁律（已回源核实 `enforcer_cached_synced.go`）：权限不一致绝不可接受，故缓存失效只能全量清。**
>
> 司域 Sidecar 用 `SyncedCachedEnforcer`（内嵌 `*SyncedEnforcer` + 自带 `SyncCache`，二者各持独立锁，`enforcer_cached_synced.go:26-31`）。它**重写**了 `AddPolicy`/`AddPolicies`/`RemovePolicy`/`RemovePolicies`——但走的是**按 key 精确删**（`checkManyAndRemoveCache:164`，key 由**规则**主体生成）；`LoadPolicy`/`InvalidateCache` 才是全量清。它**未重写** `UpdatePolicy`/`UpdatePolicies`/`RemoveFilteredPolicy`（落到内嵌 enforcer，根本不碰这层缓存），`BuildIncrementalRoleLinks` 同样不碰缓存。直接依赖它的默认失效会产生两类不一致：
> 1. **更新类不生效**：`UpdatePolicy`/`UpdatePolicies`/`RemoveFilteredPolicy` 不触达缓存，此前缓存的旧结果残留。
> 2. **RBAC 撤权不生效（结构性）**：缓存 key 由**请求**主体构成（如 `alice$$...`），而 `RemovePolicies` 只按**规则**主体删 key（如 `manager$$...`，`checkManyAndRemoveCache:172` 用 rule 构造 key）。alice 经 `manager` 角色获得的 `true` 缓存对不上、不会被删——撤权后 alice 仍能访问。按 key 失效在角色间接性下**无法正确**。
>
> **因此司域的硬性规则：Sidecar 同步协程每应用一条 delta（`AddPolicies`/`RemovePolicies`/`UpdatePolicy`，g 段由高层 API 自动 `BuildIncrementalRoleLinks`）后，必须调用 `InvalidateCache()` 全量清空决策缓存。** 不依赖按 key 删（RBAC 下不可靠），不依赖 TTL 做一致性（TTL=N 秒 ≡ 允许 N 秒权限不一致，违反底线；TTL 仅作内存上界）。代价：策略变更后缓存短暂回冷、回落 O(n) 扫描，但策略变更频率远低于鉴权请求，很快重新焐热，是正确性优先的合理取舍。
>
> **改内存与清缓存之间的并发窗口（可接受的新鲜度滞后，非一致性破坏）：** `SyncedCachedEnforcer` 命中缓存时只走 cache 锁返回（`Enforce:60-83`），不取策略 RWMutex——因此"同步协程已 apply delta 但尚未 `InvalidateCache()`"的极短窗口内，并发鉴权可能读到旧的 `true`。这属于 [2.2 最终一致时序容忍](#22-关键架构原则)：窗口在同一协程内顺序执行（apply → 紧接 InvalidateCache），跨度微秒级，且每条 delta 都以全量清收口，不存在"撤权后长期仍可访问"。要彻底消窗需让鉴权与 apply 共用一把锁、牺牲并发吞吐——按"一致性 > 性能、但新鲜度滞后可容忍"裁决，此处不消窗，仅在文档显式标注。
...
DataPolicy 不进 casbin 的 `p`/`g` 段（casbin 只做 true/false，不存条件树），因此**它不在 casbin 内存模型里、也不受 casbin 决策缓存管辖**。它需要司域自建一条与功能权限**对等的同步+失效链路**，同样守"权限不一致绝不可接受"底线：

- **存储与下发**：DataPolicy 在控制面与功能策略**同库、共用同一条单调版本号序列**（按 app 维度）。变更走与功能策略**同一条 gRPC delta 通道**推送，delta 标注类型（`data_policy_add`/`update`/`remove`），Sidecar 落到独立的内存 DataPolicy 表（按 `subject→[]DataPolicy` 索引）。版本号与功能策略统一，心跳对账（[7 节](#7-策略同步机制)）一并覆盖——丢包时全量快照拉取同时重建 casbin 策略与 DataPolicy 表。
- **求值结果不缓存（首期）**：`/filter` 的结果依赖 `user_attrs`（运行时变量，每请求不同），缓存命中率低且 key 空间大；首期**不缓存渲染结果**，每次现算（条件树求值 + 方言渲染，纯内存 CPU 操作，无 n 量级扫描）。这样 DataPolicy 一旦 apply 即刻生效，无失效窗口。
- **若后续引入 `/filter` 缓存**：必须复用功能权限的失效铁律——**DataPolicy 表每应用一条 delta 后全量清** `/filter` 缓存，且 key 不能只按规则主体（同 RBAC 角色间接性问题）。在未论证清楚失效正确性前，不引入该缓存。
- **主体解析共享角色图**：求值入口 `GetImplicitRolesForUser` 读的是 casbin 内存角色图——意味着**角色关系变更（g 段 delta）同时影响功能权限与数据权限主体解析**。两者共用同一份角色图、同一次 `BuildIncrementalRoleLinks`，天然一致，无需双写。
...
gRPC 推送服务 (双向 stream)
    │
    │  2a. 实时推送 delta（变更类型 + 规则 + 版本号）
    │  2b. 周期心跳推送本 app 当前 max 版本号（~30s，反熵）
    ▼
Sidecar 策略同步协程
    │
    │  3. 按 delta 直接改内存策略（不回源全量 LoadPolicy）
    ▼
casbin 内存操作（高层 API，autoSave=false 不回写 adapter）：
    AddPolicies / RemovePolicies / UpdatePolicy
    （g 段由高层 API 自动 BuildIncrementalRoleLinks，无需手动调）
    + InvalidateCache（全量清决策缓存，见 5 节铁律）
    │
    │  4. 比对版本号：last-applied 与 delta/心跳 版本号不连续 →
    │     向控制面发起一次「全量快照拉取」(自定义 gRPC RPC) 对齐
    ▼
鉴权 API（使用新策略）
```

> **版本对账兜底（C2 / R3，心跳 + 全量快照拉取）：** 仅靠"delta 版本号不连续"检测丢包有一个盲区——**末条变更丢失且此后无新变更**：Sidecar 永远等不到下一条 delta，无从发现自己落后。因此控制面**周期性（~30s）心跳推送每个 app 的当前 max 版本号**（反熵）。Sidecar 比对 last-applied：
> - 实时 delta 版本号跳变（中间丢包）→ 立即拉全量快照；
> - 心跳版本号 > last-applied（末条丢失/长期静默丢包）→ 拉全量快照；
> - 一致 → 无操作。
>
> 这一机制零外部依赖、覆盖"末条丢失且静默"场景，且把对账成本压在低频心跳上。Redis Pub/Sub 的 at-most-once 丢失由此收口（见 [4.1](#41-控制面高可用i4)）。

> **「全量快照拉取」不是 casbin `LoadFilteredPolicy`（R10）：** Sidecar 的 `MemoryAdapter` 只在启动时加载一次本地快照、`autoSave=false`，运行期策略全部经 delta 改内存——它**不连控制面**。因此对账时的"全量对齐"是司域**自定义的 gRPC RPC**：Sidecar 调控制面「拉取本 app 全量策略 + 当前版本号」，用返回结果**重建内存模型**（`ClearPolicy` → `AddPolicies` 批量灌入 → `InvalidateCache`），而非调 casbin `LoadFilteredPolicy`（后者只会从 Sidecar 自己的 `MemoryAdapter` 即启动快照里读，根本到不了控制面）。casbin 的 `FilteredAdapter`/`LoadFilteredPolicy` 仅用于**启动时**按 domain 分片加载本地快照。

> **为什么不用默认 `Watcher`（C2）：** 已回源核实 casbin 三个相关接口（`persist/watcher.go`、`watcher_ex.go`、`enforcer.go:252` `SetWatcher`）：
> - **`Watcher`（基础）**：`SetWatcher` 默认把回调设为 `func(string){ _ = e.LoadPolicy() }`——收到通知即**全量重载**，高频变更下抖动大、且与"分片内存"模型冲突。**不用。**
> - **`WatcherEx`（带 delta）**：`UpdateForAddPolicies` 等方法携带具体变更规则；但 `SetWatcher` 对 WatcherEx **故意不设默认回调**（源码注释："has no generic implementation"）——即 casbin 只负责把 delta 广播出去，**接收端如何 apply 完全交给实现方**。Sydom 正是落在这个空位上：Sidecar 收到 delta 后直接调高层内存增删改 API（`AddPolicies`/`RemovePolicies`/`UpdatePolicy`），g 段策略由这些 API 自动 `BuildIncrementalRoleLinks`（`internal_api.go:111`），只重建受影响的角色链，不回源、不全量重建。
> - **接收端 enforcer 不挂 watcher（R6，避免回播）：** Sidecar 的 enforcer 是纯订阅端，绝不 `SetWatcher`。casbin 高层写 API 在 `autoNotifyWatcher` 开启时会回调 watcher 广播（`internal_api.go` notify 链）；Sidecar 既不设 watcher、apply 时也保持 `autoNotifyWatcher=false`，从根上杜绝"收到 delta→apply→又广播出去"的回播环。
>
