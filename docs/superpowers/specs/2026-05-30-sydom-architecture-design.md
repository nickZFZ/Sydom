...
- **零外部依赖（默认形态）**：数据面是"多 Sidecar 对一控制面"，策略变更需广播到所有订阅的 Sidecar。MVP 不引入 Redis/Kafka 等外部组件，用**数据库自身**做广播总线：
  - **PostgreSQL**：`LISTEN/NOTIFY`，策略变更事务提交后 `NOTIFY`，各控制面副本监听后向自己持有的 Sidecar stream 推 delta。
  - **MySQL**：无原生 pub/sub，副本**轮询变更表**（按版本号增量拉取）后推送，牺牲一点实时性换取零依赖。
- **广播层可插拔升级**：当 Sidecar 规模增长到 DB 广播成为瓶颈时，把广播总线替换为 Redis Pub/Sub 或 NATS，控制面与 Sidecar 协议不变。这是"先零依赖、按规模演进"，而非一步到位上重组件。

> **与 casbin 分布式机制的关系：** casbin 的 `Watcher`（通知-重载，最终一致）和 `Dispatcher`（状态机复制，强一致，需 Raft）都是面向"多个**对等 enforcer 实例**之间同步策略"。司域是**控制面单写、Sidecar 只读订阅**的拓扑——写入只发生在控制面（落 DB），Sidecar 不回写，因此不需要 Dispatcher 的 Raft 强一致，最终一致（< 5s）即可。控制面副本之间无需互相同步策略状态（状态在 DB），只需共享一条广播通道把变更扩散给 Sidecar。
...
> **为什么不用 `Watcher` + `LoadPolicy()`（C2）：** casbin 默认 `Watcher` 触发全量 `LoadPolicy()`，高频变更下抖动大、且与"分片内存"模型冲突。Sydom 走 casbin **Dispatcher 路线的精神**——推送 delta 后直接调内存增删改 API（`AddPolicies`/`RemovePolicies`/`UpdatePolicy`）并 `BuildIncrementalRoleLinks`，只重建受影响的角色链，不回源、不全量重建。版本号用于检测丢包：Sidecar 发现版本不连续时，回退到一次 `LoadFilteredPolicy`（只拉本 domain 分片）做兜底对齐。
