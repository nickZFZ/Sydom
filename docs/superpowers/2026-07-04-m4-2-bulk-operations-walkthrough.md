...
- **✅ 真实浏览器 axe 走查（2026-07-05 完成）**：一次性 build-tag `walkthrough` 脚手架（`zz_walkthrough_scaffold_test.go`，复用 `newConsole` 装配 + `dbtest` testcontainers PG+Redis，root 超管、会话 TTL `time.Hour`、播种使 5 页非空、URL 写文件）+ 系统 Chrome via **Playwright MCP**（先修复了 Playwright MCP：`@latest` 冷启动因本机无 npm registry 访问超 30s 超时掉线 → 钉 `--prefer-offline @playwright/mcp@0.0.77`）+ **axe-core 4.10.2**（jsdelivr 取、本地 `python3 -m http.server` 服、页内 `<script src>` 注入）。root 同源 `fetch('/login')` 拿 HttpOnly 会话 → 逐页 `page.goto` 真实渲染 → `axe.run(document,{resultTypes:['violations']})`。脚手架/axe 静态服务走查后已删/停、未提交。

  | 页/状态 | 路由 | axe 4.10.2 违规 | 单 h1 | breadcrumb | 关键（真实渲染核实） |
  |---|---|---|---|---|---|
  | 角色 | `/apps/1/roles` | **0** | ✓「角色」 | ✓ | 3 复选框 `aria-label` 唯一（选择角色 manager/clerk/auditor）+ 批量 form |
  | 授权 | `/apps/1/grants` | **0** | ✓「授权」 | ✓ | 复合值 `1:1`/`1:2`/`2:1`（role_id:permission_id） |
  | 角色继承 | `/apps/1/inheritances` | **0** | ✓「角色继承」 | ✓ | 复合值 `2:1`/`3:1`（child:parent） |
  | 用户绑定 | `/apps/1/bindings` | **0** | ✓「用户绑定」 | ✓ | 复合值 `alice@corp:1`/`bob@corp:2`（user_id:role_id） |
  | 数据策略 | `/apps/1/data-policies` | **0** | ✓「数据策略」 | ✓ | `aria-label`「选择数据策略 role manager 对 order」——subject 判别唯一（我修的唯一性收敛真实渲染确认） |
  | 服务端确认页 | `/apps/1/roles/batch-delete` | **0** | ✓（专属文案作 h1「确认批量移除选中的角色？将一并移除其授权与绑定。」） | 过渡页无（M3.4a 既有设计） | 隐藏 `ids=[1,2]` 回显 + `confirmed=1` 字段 + 确认按钮 |

  **端到端流程（真实浏览器）**：勾选 2 行 → 提交（`form.submit()` 绕 JS 弹窗模拟无 JS 路径）→ 服务端确认页（专属业务文案，confirmPrompts 命中，非通用兜底）→ confirmed=1 → **303 PRG 回列表** → **真实删除**（manager+clerk 移除，列表只剩 auditor；再删 auditor 列表清空）→ **flash toast**（`<div class="toast" role="status" aria-live="polite" data-toast>`「已移除选中项」，服务端渲染 HTML 直接取证；页面 JS 4s 后自动淡出，非 bug）。

## 裁决

**READY 且真实浏览器 axe 走查已完成**。BC-1..8 逐条满足，全量套件绿，授权核心零触碰经空白归一化严格证明；6 个页面/状态（5 列表页 + 服务端确认页）真实浏览器 axe-core 4.10.2 实测**各 0 违规**，多选→确认→PRG→删除→flash 全链路真实浏览器验证通过。M4.2 全部关卡闭合。
